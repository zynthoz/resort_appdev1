...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO tbl_reservations (user_id, accommodation_id, check_in_date, check_out_date, total_price, reservation_status) VALUES ($user_id, $accommodation_id, '$check_in', '$check_out', $total, 'pending')";
...
<div class="main-content">
    <div class="page-header"><h1>Add Reservation</h1><p>Create a reservation on behalf of a customer</p></div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="accommodation_id" name="accommodation_id" required>
                        ...
                        <?php while ($a = mysqli_fetch_assoc($accommodations)): ?>
                            <option value="<?= $a['accommodation_id'] ?>" data-price="<?= $a['price_per_night'] ?>"><?= htmlspecialchars($a['accommodation_name']) ?> — ₱<?= number_format($a['price_per_night'], 2) ?>/night</option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="row mb-3">
                    <div class="col-md-6"><label for="check_in_date" class="form-label">Check-in Date</label><input type="date" class="form-control" id="check_in_date" name="check_in_date" required></div>
                    <div class="col-md-6"><label for="check_out_date" class="form-label">Check-out Date</label><input type="date" class="form-control" id="check_out_date" name="check_out_date" required></div>
                </div>
                <div class="total-display mb-4"><span class="total-label">Estimated Total</span><span class="total-amount" id="total-price">₱0.00</span></div>
                <div class="d-flex gap-2"><button type="submit" class="btn btn-primary">Create Reservation</button><a href="reservations.php" class="btn btn-outline-secondary">Cancel</a></div>
            </form>
        </div>
    </div>
</div>
...
function calcTotal() {
    ...
    if (checkIn && checkOut && checkOut > checkIn) {
        ...
        document.getElementById('total-price').textContent = '₱' + (nights * price).toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    } else { document.getElementById('total-price').textContent = '₱0.00'; }
}
